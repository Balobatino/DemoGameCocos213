...
export class MainGameBoostrapper extends Singleton<MainGameBoostrapper> {
    ...
    public mainBGM: cc.AudioClip | null = null;

    protected doOnStart(): void {
        // Start the full async flow without blocking the engine.
        // console.log(`MainGameBoostrapper: doOnStart called. time ${Date.now() / 1000}`);
    ...
    private async runProcess(): Promise<void> {
        // console.log(`MainGameBoostrapper: runProcess started. time ${Date.now() / 1000}`);
        ...
        if (loadingCover) {
            // console.log(`MainGameLoadingCover: runFadeOut called. time ${Date.now() / 1000}`);
