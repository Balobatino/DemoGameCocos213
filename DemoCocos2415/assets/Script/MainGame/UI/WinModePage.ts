 ...
 */
@ccclass("WinModePageUIReference")
...
/**
 * WinModePage: Singleton that manages the mode completion UI page.
 */
@ccclass
export default class WinModePage extends Singleton<WinModePage> {
    ...
    //------------------------------
    //--- Private Properties
    // Cached UIPage component for the win mode page.
    ...
    private cacheComponents(): void {
        ...
        if (!this.uiPage) {
            console.error(`WinModePage: UIPage component not found on node "${this.node.name}" or its children.`);
        }
    }

    private registerButtonHandlers(): void {
        const { levelSelectButton, gameModeSelectButton } = this.uiRef;

        if (levelSelectButton) {
            levelSelectButton.node.on(cc.Node.EventType.TOUCH_END, this.onLevelSelectClicked, this);
        } else {
            console.warn("WinModePage: levelSelectButton is not assigned.");
        }
    ...
    /**
     * Hides the current win mode page and opens a target page after a delay.
    ...
    private navigateToPage(targetGetter: () => Singleton<any> & { getUiPage: () => UIPage | null }): void {
        ...
        this.scheduleOnce(() => {
            ...
            if (!target) {
                console.warn("WinModePage: Target singleton instance not found.");
                return;
            }
            ...
            if (targetUiPage) {
                ...
                console.warn("WinModePage: Target UIPage component not found.");
            }
        }, delay);
    }
