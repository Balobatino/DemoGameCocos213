 ...
 * Adapted for Cocos Creator 2.x (uses `cc.Sprite`, node opacity, and `cc.tween`).
 */
@ccclass
export class MainGameLoadingCover extends Singleton<MainGameLoadingCover> {
    ...
    public coverScreen(): void {
        if (!this.screenCover) return;
        // Ensure sprite is enabled (in Editor it may be disabled by default so it won't block view).
        this.screenCover.node.active = true;
    ...
    public runFadeOut(onComplete?: () => void): void {
        ...
        // Ensure sprite is active (in Editor it may be disabled by default).
        this.screenCover.node.active = true;
    ...
    public runFadeIn(onComplete?: () => void): void {
        ...
        // Ensure sprite starts black; keep it transparent via node opacity (not color alpha).
